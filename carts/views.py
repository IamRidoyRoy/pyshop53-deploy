...
def add_cart(request, product_id):
    # By using it we get the product.
    product = Product.objects.get(id=product_id)

    # Solve increase, decrease and variation issue
        ...
        if is_cart_item_exists:
            ...
            if product_variation in ex_var_list:
                ...
                if len(product_variation) > 0:
                    item.variations.clear()  # It will clear the pervious variations
                    item.variations.add(*product_variation)
                # cart_item.quantity += 1 #cart quantuty will increment by one
                item.save()
        else:
    ...
    # if the user is not authenticated
        ...
        try:
            # get cart using cart id in the session
